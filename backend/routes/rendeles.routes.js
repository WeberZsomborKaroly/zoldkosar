...
router.post('/szallitasi-adatok', [authJwt.verifyToken], async (req, res) => {
    try {
        ...
        // Felhasználó frissítése a szállítási adatokkal
...
// Root endpoint for order creation - to match what the frontend is using
router.post('/', [authJwt.verifyToken], async (req, res) => {
    ...
    try {
        ...
        const felhasznaloId = req.userId || 1; // Use userId from token if available, otherwise default to 1
