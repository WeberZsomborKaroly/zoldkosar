...
const OrderForm = ({ onSubmit, onCancel, total, appliedCoupon, discountAmount }) => {
    ...
    const calculateShippingCost = (megye) => {
        if (total >= 15000) return 0; // Ingyenes szállítás 15000 Ft felett
        if (megye === 'Zala') return 0; // Ingyenes szállítás Zala megyében
    ...
    // Felhasználó adatainak és mentett szállítási adatainak betöltése
    useEffect(() => {
        const loadUserData = async () => {
            try {
                ...
                // Mentett szállítási adatok lekérése localStorage-ból
                try {
                    ...
                    if (savedShippingData) {
                        ...
                        // Formdata frissítése a mentett adatokkal
                        ...
                        // Szállítási költség számítása a betöltött megyével
    ...
    // Calculate subtotal (before discount) when total or discount changes
    useEffect(() => {
        if (appliedCoupon) {
    ...
    const handleFinalSubmit = async (e) => {
        ...
        try {
            // Szállítási adatok mentése localStorage-ba
            ...
            // Form adatok továbbítása a szülő komponensnek a szállítási költséggel együtt
            onSubmit({
                ...formData,
                szallitasiKoltseg
            });
    ...
    // Megfelelő űrlap megjelenítése az aktuális lépés alapján
    if (showCardForm) {
