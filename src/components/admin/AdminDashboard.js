...
const AdminDashboard = () => {
    ...
    // Ellenőrizzük, hogy az aktuális útvonal aktív-e
    const isActive = (path) => {
...
const Overview = () => {
        ...
        const fetchLowStockProducts = async () => {
            try {
                ...
                const lowStock = productsResponse.data
                    .filter(product => product.keszlet < 20)
                    .map(product => ({
                        id: product.id,
                        name: product.nev || 'Ismeretlen termék',
                        stock: product.keszlet || 0,
                        price: product.ar || 0,
                        category_id: product.kategoria_id
                    }));
                ...
                // Add category names to low stock products
                const productsWithCategories = lowStock.map(product => {
                    const category = categories.find(c => c.id === product.category_id);
                    return {
                        ...product,
                        category: category ? category.nev : 'Ismeretlen kategória'
                    };
                });
                
                setLowStockProducts(productsWithCategories);
            } catch (error) {
                console.error('Hiba az alacsony készletű termékek lekérése során:', error);
            }
        };
    ...
    return (
        <div className="overview">
            ...
            {/* Alacsony készletű termékek szekció */}
