...
const Profile = () => {
    ...
    // State a visszaküldési modál kezeléséhez
    ...
    const loadUserData = async () => {
        try {
            ...
            // Try to load saved shipping data if available
            try {
                // Mivel a szerver végpont még nem létezik, használjunk localStorage-ot
            ...
            // Set invoice data from user data
            setInvoiceData({
                adoszam: userData.adoszam || '',
                ceg_nev: userData.ceg_nev || '',
                szallitasi_cim: ''
    ...
    // Rendelések betöltése a felhasználó email címe alapján
    const loadOrders = async () => {
        try {
            ...
            // Az admin végponton keresztül kérjük le az összes rendelést
            const response = await axiosInstance.get('/admin/orders');
            console.log('Összes rendelés:', response.data);
            
            if (response.data && Array.isArray(response.data)) {
                // Szűrjük a rendeléseket a felhasználó email címe alapján
                const userOrders = response.data.filter(order => {
                    try {
                        // Ellenőrizzük, hogy a rendelés szállítási címe tartalmazza-e az email címet
                        if (typeof order.szallitasi_cim === 'string') {
                            if (order.szallitasi_cim.includes(userData.email)) {
                                return true;
                            }
                            
                            // Próbáljuk meg JSON-ként is értelmezni
                            try {
                                ...
                                // JSON parsing hiba, de ez nem gond
                            }
                        }
                        
                        // Ellenőrizzük az email mezőt is, ha létezik
                ...
                // Lekérjük minden rendeléshez a tételt
    ...
    const handlePersonalDataSubmit = async (e) => {
        ...
        try {
            ...
            // Mivel a szerver végpont még nem létezik, csak frissítsük a localStorage-ot
            // const response = await axiosInstance.put('/felhasznalo/profil', personalData);
            ...
            // Update invoice data with the new values
            setInvoiceData({
    ...
    const handleShippingDataSubmit = async (e) => {
        e.preventDefault();
        try {
            // Mivel a szerver végpont még nem létezik, mentsük el localStorage-ba
            // await axiosInstance.post('/felhasznalo/szallitasi-adatok', formData.szallitasi_adatok);
    ...
    const handleReturnOrder = async (order) => {
        try {
            // Mivel a szerver végpont még nem létezik, csak logoljuk a kérést
    ...
    // Visszaküldési modál megnyitása
    const openReturnModal = (order) => {
        setReturnOrder(order);
        setReturnModalOpen(true);
    };

    // Visszaküldési modál bezárása
    ...
    const generateInvoicePDF = () => {
        try {
            ...
            // Biztosítsuk, hogy a termékek tömb legyen
            ...
            // Process products - ellenőrizzük, hogy a termékek tömb-e
            let productRows = '';
            ...
            if (Array.isArray(selectedOrder.termekek) && selectedOrder.termekek.length > 0) {
                productRows = selectedOrder.termekek.map(item => {
                    ...
                    // Biztosítsuk, hogy az egysegar szám
                ...
                // Ha nincs termék adat, akkor egy üres sor
                productRows = `
                    <tr>
                ...
                // Ha nincs termék adat, de van összeg, akkor azt használjuk
        ...
        <div className="orders-section">
            ...
            {orders && orders.length > 0 ? (
                <div className="orders-list">
                    {orders.map(order => {
                        if (!order) return null; // Skip if order is undefined
                        ...
                        // Rendelés tételek - biztosítsuk, hogy mindig tömb legyen
                        const orderItems = Array.isArray(order.termekek) ? order.termekek : [];
                        
                        // Biztosítsuk, hogy a statusz mindig string legyen és megfelelő formátumban
                        let orderStatus = (order.statusz || order.allapot || 'feldolgozas_alatt').toString();
                        
                        // Statusz formázása a CSS-hez - eltávolítjuk az ékezeteket és a szóközöket
                        // pl. "szállítás alatt" -> "szallitas_alatt"
                        orderStatus = orderStatus.normalize("NFD").replace(/[\u0300-\u036f]/g, "") // Eltávolítjuk az ékezeteket
                                                .replace(/\s+/g, '_')       // Szóközöket aláhúzásjelre cseréljük
                                                .replace(/-/g, '_')         // Kötőjeleket aláhúzásjelre cseréljük
                                                .toLowerCase();             // Kisbetűsre alakítjuk
                                ...
                                <div className="order-items">
                                    {orderItems.length > 0 ? (
                                        orderItems.map((item, index) => {
                                            ...
                                            // Biztosítsuk, hogy van termék név és ár
